...
public class Juego {
	...
	public static void main(String[] args) {
		...
		int x=1;
		while(x!=0 && partida.Posiblejugada(partida.newcartas[0].getNumero())){
			int j=1;System.out.println("		Record:"+partida.arecord+"	Score:"+partida.ascore+"		X"+partida.amultiplicador);
			while(j<5) {
				int i=0;
				System.out.println("aColumna "+j);
				System.out.print(partida.aColumnas[j].cantidad+"  -> ");
				while(partida.aColumnas[j].cartas[i]!=null){
					System.out.print(partida.aColumnas[j].cartas[i].valor()+"/");
					i++;
				}
				System.out.println("		");
				j++;
			}
			j=1;System.out.println("		Record:"+partida.record+"	Score:"+partida.score+"		X"+partida.multiplicador);
			while(j<5) {
				int i=0;
			...
			System.out.println("		Cartas: "+(int) (Math.pow(2,partida.newcartas[1].getNumero()))+" '"+(int) (Math.pow(2,partida.newcartas[0].getNumero()))+"'");
			...
			x= sc.nextInt();
			if(x>0 && x<5) {
				if(partida.Columnas[x].cantidad<8 || partida.Columnas[x].ultima==partida.newcartas[0].getNumero()) {
					partida.Guardar();
					partida.AgregarCarta(x);
